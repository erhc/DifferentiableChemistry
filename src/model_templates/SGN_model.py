...
def SGN_model(model_name: str, layers: int, node_embed: str, edge_embed: str, connection: str, param_size: int, **kwargs):
    ...
    def get_sgn(layer_name: str, node_embed: str, param_size: tuple):
        ...
        template += [R.get(f"{layer_name}_order_1")(V.X, V.Y) <= (R.get(connection)(V.X, V.Y, V.B),
        ...
        for i in range(2, max_depth+1):
            ...
            # template += [head <= body]
            template += [R.get(f"{layer_name}_order_{i}")(V.X, V.Y) <= 
        ...
        template += [R.get(layer_name)(V.X) <= (R.get(f"{layer_name}_order_{max_depth}")(V.X, V.Y)),
                     R.get(layer_name)(V.Y) <= (R.get(f"{layer_name}_order_{max_depth}")(V.X, V.Y))]
        return template
