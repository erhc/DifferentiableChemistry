    ...
    **kwargs,
    ...
    def bond_features(layer_name: str, prev_layer: str, param_size: tuple):
        ...
        def get_bond_cycle(n):
            body = [
                R.get(connection)(f"X{i}", f"X{(i + 1) % n}", f"B{i}") for i in range(n)
            ]
            body.extend(
                R.get(f"{prev_layer}_edge")(f"B{i}")[param_size] for i in range(n)
            )
    ...
    def get_cw(layer_name: str, prev_layer: str, param_size: tuple):
        ...
        template += [
            R.get(layer_name)(V.X) <= (R.get(layer_name + "_node")(V.X)[param_size])
        ]
        template += [
            R.get(layer_name)(V.X) <= (R.get(layer_name + "_edge")(V.X)[param_size])
        ]
    ...
    if param_size == 1:
        template += [(R.get(output_layer_name)[param_size,] <= R.get(f"{model_name}_cw")(V.X))]
        param_size = (param_size,)
    else:
        template += [(R.get(output_layer_name)[1, param_size] <= R.get(f"{model_name}_cw")(V.X))]
    ...
    for i in range(layers):
        ...
        template += get_cw(
            f"{model_name}_cw_{i + 1}", f"{model_name}_cw_{i}", param_size
        )

    # Write output layer and prediction
    template += [
        (R.get(f"{model_name}_cw")(V.X) <= R.get(f"{model_name}_cw_{layers}")(V.X))
    ]
