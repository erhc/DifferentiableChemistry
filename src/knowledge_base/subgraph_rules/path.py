...
def get_path(layer_name: str, node_embed:str, edge_embed: str, connection: str, param_size: tuple, max_depth: int):
    template = []
    ...
    # Base case
    template += [R.get(f"{layer_name}_path")(V.X, V.Y, 0) <= (R.get(connection)(V.X, V.Y, V.B),
                                                              R.get(edge_embed)(V.B)[param_size],
                                                              R.get(node_embed)(V.Y)[param_size])]
    # Recursive calls
    template += [R.get(f"{layer_name}_path")(V.X, V.Y, V.T) <= (R.special.next(V.T1, V.T),
    ...
    # Aggregating for X
    template += [R.get(f"{layer_name}_pattern")(V.X) <= R.get(f"{layer_name}_path")(V.X, V.Y)[param_size]]
