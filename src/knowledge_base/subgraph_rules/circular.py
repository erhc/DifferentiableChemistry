...
def get_circular(
    ...
    template.append(
        R.get(f"{layer_name}_heterocycle")(V.X)
        <= (
            R.get(carbon)(V.C),
    ...
    template += [
        R.get(f"{layer_name}_circular")(V.X) <= R.get(subgraph)(V.X)[param_size]
        for subgraph in [f"{layer_name}_brick", f"{layer_name}_heterocycle"]
    ]
    template += [
        R.get(f"{layer_name}_pattern")(V.X)
        <= R.get(f"{layer_name}_circular")(V.X)[param_size]
    ]
