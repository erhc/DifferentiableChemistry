...
def get_y_shape(layer_name: str, node_embed:str, edge_embed: str, connection: str, param_size: tuple, double_bond: str):
    template = []
    
    # Aggregating messages in a double bond
    template += [R.get(f"{layer_name}_double_bond_subgraph")(V.X) <= (
    ...
    template += [R.get(f"{layer_name}_y_group")(V.X1, V.X2, V.X3) <= (
    ...
    template += [
        ...
        (R.get(f"{layer_name}_y_bond_patterns")(V.X1) <= R.get(f"{layer_name}_y_bond")(V.X1, V.X2, V.X3, V.X4)[param_size]),
