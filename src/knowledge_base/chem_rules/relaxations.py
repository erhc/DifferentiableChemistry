...
def get_relaxations(layer_name: str, param_size: tuple, connection: str, carbon: str):
    ...
    template += [
        R.get(f"{layer_name}_relaxed_aromatic_bonded")(V.X, V.Y)
    ...
    template += [
        R.get(f"{layer_name}_relaxed_carbonyl_group")(V.X, V.Y)
        <= (
    ...
    template += [
        R.get(f"{layer_name}_relaxed_carbonyl_group")(V.C, V.O, V.R1, V.R2)
        <= (
    ...
    template += [
        R.get(f"{layer_name}_relaxed_benzene_ring")(V.A)
        <= R.get(f"{layer_name}_relaxed_benzene_ring")(V.A, V.B)
    ]
    template += [
        R.get(f"{layer_name}_relaxed_benzene_ring")(V.A, V.B)
    ...
    template += [
        R.get(f"{layer_name}_potential_group")(V.C)
        <= (
    ...
    template += [
        R.get(f"{layer_name}_carbonyl_derivatives")(V.X)
        <= (
    ...
    template += [
        R.get(f"{layer_name}_carbonyl_derivatives")(V.C)
        <= (
    ...
    template += [
        ...
        <= (
            R.get(f"{layer_name}_relaxed_carbonyl_group")(V.X, V.Y, V.R1, V.R2)[
                param_size
            ]
        )
    ]
