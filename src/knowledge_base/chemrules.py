...
from knowledge_base.chem_rules.general import get_general
from knowledge_base.chem_rules.hydrocarbons import get_hydrocarbons
from knowledge_base.chem_rules.nitro import get_nitro
from knowledge_base.chem_rules.oxy import get_oxy
from knowledge_base.chem_rules.relaxations import get_relaxations
from knowledge_base.chem_rules.sulfuric import get_sulfuric
...
def get_chem_rules(layer_name: str, node_embed: str, edge_embed: str, connection: str, param_size: int,
                   ...
                   path=None,
                   hydrocarbons=False, nitro=False, sulfuric=False, oxy=False, relaxations=False, key_atoms: list = []):
    template = []
    ...
    if oxy:
        ...
        template += [R.get(f"{layer_name}_functional_group")(V.X)[param_size] <= R.get(f"{layer_name}_oxy_groups")(V.X)]
    if nitro:
    ...
    if sulfuric:
        template += get_sulfuric(layer_name, param_size, carbon, hydrogen, sulfur, nitrogen)
